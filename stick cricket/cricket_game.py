...
# Load sprite images (replace with your own sprite sheet frames)
sprite_images = [
    pygame.image.load(f"D:/score_counter/stick cricket/bat_{i}.png") for i in range(1, 6)  # Assuming 4 frames
]
sprite_images = [pygame.transform.scale(img, (100, 100)) for img in sprite_images]  # Resize if needed

bowler_images = [
    pygame.image.load(f"D:/score_counter/stick cricket/bowl_{i}.png") for i in range(1, 5)  # Assuming 4 frames
]
bowler_images = [pygame.transform.scale(img, (80, 80)) for img in bowler_images]  # Resize if needed
...
class AnimatedSprite(pygame.sprite.Sprite):
    def __init__(self, x, y, cond):
        ...
        super().__init__()
        if cond == "bat":
            self.images = cycle(sprite_images)  # Cycle through frames
            self.len = len(sprite_images)
        elif cond == "bowl":
            self.images = cycle(bowler_images)  # Cycle through frames
            self.len = len(bowler_images)

        self.image = next(self.images)  # Get first frame
        self.rect = self.image.get_rect(topleft=(x, y))
        self.animating = False
        self.animation_speed = 5  # Adjust speed
        self.frame_count = 0
        self.complete_animation = False  # Track if animation should complete
    ...
    def update(self):
        ...
        if self.animating:
            ...
            # Only update the image every `animation_speed` frames
            if self.frame_count % self.animation_speed == 0:
                self.image = next(self.images)

            # Check if animation is complete
            if self.frame_count >= self.len * self.animation_speed:
                self.frame_count = 0
                self.stop_animation()
    ...
    def start_animation(self):
        ...
        self.animating = True
        self.complete_animation = True  # Ensure animation completes
        self.cooldown = 60  # Set cooldown to 1 second (60 frames)

    def stop_animation(self):
        if not self.complete_animation:
            return  # Don't stop if animation is not complete
        self.animating = False
        self.complete_animation = False  # Reset flag
# Create sprite instance
sprite = AnimatedSprite(batsman_pos[0], batsman_pos[1],"bat")
sprite_group = pygame.sprite.Group(sprite)

bowler = AnimatedSprite(bowler_pos[0], bowler_pos[1],"bowl")
bowler_group = pygame.sprite.Group(bowler)
...
def bowl():
    ...
    global ball_in_play, ball_speed, player_score
    ball_pos[:] = [680, 450]  # Reset ball position
    
    if player_score > 50:
        speed = 30
        angle = random.uniform(-10, -1)
    elif player_score > 20:
        speed = 26
        angle = random.uniform(-5, 1)  # Slight random variation in bowling angle
    else:
        speed = 16
...
def game_page(screen, player, file):
    ...
    while running:
        ...
        draw_pitch(screen)
        pygame.draw.line(screen, YELLOW, (110, 470), (110, 500), 6)  # Stump
        sprite_group.update()
        sprite_group.draw(screen)

        bowler_group.update()
        bowler_group.draw(screen)
        draw_ball(screen)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and not ball_in_play:
                    bowler.start_animation()
                    bowl()

                if event.key == pygame.K_RETURN:
                    bat_swinging = True
                    bat_angle = 0  # Swing forward
                    sprite.start_animation()  # Start animation on key press

                if event.key == pygame.K_ESCAPE:
                    add_score(player, file, h_score)
                    running = False

            # No need for KEYUP handling for animation control
